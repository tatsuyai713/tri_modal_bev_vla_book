...
| Stage 6 | Joint Fine-tuning | 全データ混合 | 全モジュール学習可（低LR） |
...
損失:
  L_joint = L_stage2 + L_stage3 + L_stage4 + L_stage5 (各損失の重み付き和)
...
## D.10 各モジュールのFreeze/Train テーブル

| モジュール | Stage1 | Stage2 | Stage3 | Stage4 | Stage5 | Stage6 |
|---|:---:|:---:|:---:|:---:|:---:|:---:|
| Camera Backbone | Train | Frozen | Frozen | Frozen | Frozen | Train(低LR) |
| LiDAR Pillar Encoder | Train | Frozen | Frozen | Frozen | Frozen | Train(低LR) |
| BEV Encoder (Fusion) | Train | Frozen | Frozen | Frozen | Frozen | Train(低LR) |
| Temporal BEV Memory | Train | Frozen | Frozen | Frozen | Frozen | Train(低LR) |
| BEV Heads | - | Train | Frozen | Frozen | Frozen | Train(低LR) |
| Dynamic Object Head | - | Train | Frozen | Frozen | Frozen | Train(低LR) |
| Agent Future Predictor | - | Train | Frozen | Frozen | Frozen | Train(低LR) |
| Lane Topology Encoder | - | Train | Frozen | Frozen | Frozen | Train(低LR) |
| Text Encoder | - | - | - | Train | Frozen | Train(低LR) |
| CondFormer | - | - | Train(簡易) | Train | Frozen | Train |
| Planner Queries (K個) | - | - | Train | Frozen | Frozen | Train |
| VLA Planner Decoder | - | - | Train | Frozen | Frozen | Train |
| Scene Tokenizer | - | - | - | - | Train | Train(低LR) |

---

## D.11 Parked/Static/Dynamic のヒューリスティックラベル生成
...
## D.13 低コスト構成での学習（R9700 + 32GB RAM）
...
Step 7: Stage 6: Joint Fine-tuning（低LR）
```

---

## D.16 Checkpoint 利用時の注意点
...
本付録で設計した要素:
  1. 学習の全体方針（5原則）
  2. 6ステージ学習計画（Backbone→Heads→Planner→Language→Scene→Joint）
