...
言語以外にも、速度計画に直結する制約をトークンとしてPlannerに与える。
...
T_user_pref（ユーザーGUI設定）:
  - overspeed_tolerance_mps（または km/h）
  - profile_mode（eco / normal / assertive）
  - hard_cap_enabled（ユーザー上限を厳格適用するか）
```

```text
設計原則:
  - 法規上限（legal）は超えない
  - ユーザー許容値は「法規内での運用マージン」に限定
  - map/sign/road_markの矛盾は保守側を優先し、最終判定は外部評価器
```
```
...
speed_profiles: [B, K, T, 3]
  - 各点で (v_target, a_x_target, phase)
  - phase ∈ {ACCEL, CRUISE, DECEL}
  - 軌跡と同時に生成し、Converterへ渡す
```
...
### 発展アーキテクチャ：DiffusionDrive（§6.10）

```text
K-query Planner は実装容易性・計算コストの低さから MVP/Phase 1 に適する。
安定稼働を確認した後、以下の理由が満たされる場合は DiffusionDrive への移行を検討する。

  K-query Planner の特徴:
    - 1 forward pass で K 候補を生成、実装シンプル
    - MHP Loss が直感的でデバッグ容易
    - 標準的な Transformer ドライバソフトウェアのことで GPU メモリが少ない

  DiffusionDrive への移行を検討するァンディション:
    - K 固定候補ではカバーできない希少シナリオが発生する
    - 停止プロファイル・ IDM ・先読めカーブ速度など縦方向挺動の精度を引き上げたい
    - 候補多様性と外部評価器の組み合わせでより貭しい安全選択を実現したい

  DiffusionDrive の詳細設計は §6.10 を参照のこと。
```
...
入力:
  ...
  - ODD status
  - confidence [B, K]
  - map/sign/road_mark/user 由来の速度制約トークン
...
チェック項目:
  ...
  6. Comfort check (軌跡曲率 vs レーン曲率プロファイルとの乖離, 横G, 加速度, ジャーク制限)
  7. Traffic light check (信号状態と進行方向が矛盾しないか)
  8. Stopline precision check (停止線手前の許容範囲に停止するか)
  9. Following stop gap check (停止時の前走車間が目標範囲内か)
  10. ODD check (測位精度、センサ健全性)
...
Final rule-based speed check:
  ...
  各候補の v_target(t) が operational_cap を超えた時点で Fail

Dynamics comfort check:
  a_y_plan(t) = v_target(t)^2 * κ(t)
  a_y_meas(t) = ego_speed(t) * ego_yaw_rate(t)
  jerk_x(t) = Δa_x(t) / Δt
  jerk_y(t) = Δa_y(t) / Δt
  横Gまたはジャークが閾値を超える候補は Fail または低スコア化

Signal / stop precision check:
  RED または該当方向のARROWなしでは停止線を越える候補を Fail
  YELLOW は停止可能距離内なら停止、停止不能なら交差点内急制動を避ける
  stop_position_error = planned_stop_x - target_stop_x
  |stop_position_error| が許容範囲を超える候補は低スコア化または Fail
  前走車停止時は actual_stop_gap が desired_stop_gap を下回る候補を Fail
