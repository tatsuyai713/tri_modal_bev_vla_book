...
本書は、Vision・LiDAR・RadarをBEV（Bird's Eye View）共通空間で統合し、外部言語指示と内部シーン要約をPlannerへ入力し、複数候補軌跡と信頼度を出力する自動運転アーキテクチャを、研究設計だけでなく実車適用・製品化まで見据えて体系化した設計書です。
...
中心思想は **Structured End-to-End** です。End-to-End学習の柔軟性を活かしながら、Static World、Dynamic World、Lane Topology、Agent Futures、Dynamic Risk Map、候補軌跡、安全ログなどの明示的な中間表現を残します。

---

## Language Editions / 言語版
...
## Core Design Principles / 主要設計思想

| Area | English | 日本語 |
|---|---|---|
| Sensor fusion | Fuse Vision, LiDAR, and Radar in a shared BEV space | Vision + LiDAR + RadarをBEV共通空間で統合 |
| World model | Separate Static World, Dynamic World, Lane Topology, and Agent Futures | Static / Dynamic / Lane Topology / Agent Futureを分離 |
| Dynamic world | Keep detection/state estimation separate from multi-modal future prediction | 現在状態推定と多モード未来予測を分離 |
| Unknown objects | Use `unknown` class plus `unknown_dynamic` flag for conservative handling | `unknown`分類と`unknown_dynamic`フラグで保守的に扱う |
| Planner | Generate K candidate trajectories and select through an external evaluator | K候補軌跡 + 外部評価器による安全側選択 |
| Control | Convert selected trajectory to steering deterministically | 選択軌跡を決定論的に舵角へ変換 |
| Teacher signal | Learn from human driving trajectories, not only lane centerlines | レーン中心線ではなく人間走行軌跡を教師にする |
| Productization | Keep explicit intermediate outputs, ODD monitoring, safety logs, and rollback gates | 中間表現・ODD監視・安全ログ・ロールバックゲートを明示 |
...
- 認識・予測・計画・制御を統合するシステムアーキテクト
- Safety, regulation, ODD, validation, and logging engineers
- 安全設計・法規対応・ODD管理・検証・ログ設計を担当するエンジニア
